...
```{r}
# Extraemos los enlaces (<a>) y sus atributos y el texto
...
ggplot(frequencies, aes(x = Tipo, y = Count, fill = Tipo)) +
  ...
  ggtitle("Frecuencia de enlaces absolutos y relativos") +
  xlab("Tipo de enlace") +
  ylab("Frecuencia") +
  theme_minimal() +
...
barplot(
  ...
  xlab = "Tipo de enlace",ylab = "Suma",col = c("cadetblue2", "deepskyblue1"),
  border = "black"
)
...
pie1 <- pie(status_counts,labels = status_labels,
    col = rainbow(length(status_counts)),main = "Porcentajes de estados de los enlaces"
)
...
# Llamamos a los tres gráficos
{
  ...
  # Gráfico 3
  pie(status_counts, labels = status_labels, col = rainbow(length(status_counts)),
      main = "Porcentajes de estados")
}
```
